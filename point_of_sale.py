...
class SalePoint:
    def __init__(self, master, *args, **kwargs):
        self.master = master
        self.master.bind('<Down>', self.ajax)# for search
        self.master.bind('<Up>', self.add_to_cart)# for add to cart
        self.master.bind('<Return>', self.generate_bill)# for generate bill button 
        ...
        self.heading = Label(self.left, text="Baby Garments & Shoes", font="arial 40 bold", bg="wheat", fg="black")
    ...
    def ajax(self, *args, **kwargs):
        ...
        # print(self.r)
        if not self.r:
            tkinter.messagebox.showerror("Error", "Product with ID {} does not exist.".format(self.get_id))
        ...
        self.productname.configure(text="Product Name: "+str(self.get_name).upper())
        ...
        self.when_enter_item_name = Label(self.left, text="Note: If product name is diffeernt then enter item name", font="arial 13 bold", bg="wheat", fg="red")
        self.when_enter_item_name.place(x=0, y=310)

        self.Item_name = Label(self.left, text="Enter Item Name", font="arial 17 bold", bg="wheat", fg="black")
        self.Item_name.place(x=0, y=340)

        self.Item_name_E = Entry(self.left, width=23, font="arial 18 bold", bg="lightblue")
        self.Item_name_E.place(x=190, y=340)
        ...
        self.add_to_cart_btn = Button(self.left, text="Add To Cart", width=10,font="arial 20 bold", height=3,fg="black", bg="orange", command=self.add_to_cart)
        self.add_to_cart_btn.place(x=500, y=335)
    ...
    def add_to_cart(self, *args, **kwargs):
        ...
        if self.quantity_value > int(self.get_stock):
            ...
            # Use product name as item name if Item_name_E is empty
            item_name = self.Item_name_E.get() or self.get_name
            ...
            self.products_list.append(item_name)
            ...
            self.when_enter_item_name.place_forget()
            self.Item_name.place_forget()
            self.Item_name_E.place_forget()
    ...
    def generate_bill(self, *args, **kwargs):
        ...
        full_invoice_directory = os.path.join(current_directory, invoice_directory)
        if not os.path.exists(invoice_directory):
            os.makedirs(full_invoice_directory)

        # Generate a more understandable filename
        file_name = f"{full_invoice_directory}/Invoice_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.rtf"
        ...
        table = PrettyTable()
        table.field_names = ["SNO.", "PRODUCT NAME", "QTY", "AMOUNT"]

        for i, (product, quantity, price) in enumerate(zip(self.products_list, self.product_quantity, self.product_price), start=1):
            table.add_row([i, str(product)[:20], quantity, price])
        ...
        tkinter.messagebox.showinfo("Success", "Bill has been genrated and stock has been decreased")
